...
let highlight_state;
const highlight_translate = {"submission": "Submission", "journal": "Journal", "title": "Title", "user": "Username"};
...
function loadHighlight(keys) {
	...
	keys.forEach((item, i) => {
		...
		const colorCol = jQuery("<td>").text(item.color).css("background-color", item.color).appendTo(row);
...
function saveHighlight() {
	...
	if (!Array.isArray(highlight_state)) return Promise.resolve();

	// Save to storage
	return setKey("highlighter_keys", highlight_state).then(() => loadHighlight(highlight_state));
}
...
function addNewHighlight() {
	...
	highlight_state.push({type, field, text, color});
